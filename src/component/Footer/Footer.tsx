import Image from 'next/image'
import React from 'react'
import { CiMail } from 'react-icons/ci'
import { FaFacebookF, FaInstagram, FaLinkedinIn, FaLocationDot, FaXTwitter } from 'react-icons/fa6'
import { MdLocalPhone } from 'react-icons/md'

const Footer = () => {
  return (
    <section>
      <div className='container'>

        <div className='flex flex-col md:flex-row md:justify-between gap-10 border-y-2 py-10 border-red-400 '>
          <div className='mx-auto'>
            <h3 className='font-bold text-2xl cinzel'>Contact</h3>
            <div className='flex flex-col gap-6 text-[16px] pt-6'>
              <div className='flex gap-2 items-center '>
                <MdLocalPhone className='text-lg ' />
                <span className='font-normal'>1234567890</span>
              </div>
              <div className='flex gap-2 items-center  '>
                <FaLocationDot className='text-lg ' />
                <span className='font-normal '>Your Address Here</span>
              </div>

              <div className='flex gap-2 items-center '>
                <CiMail className='text-lg ' />
                <span className='font-normal  '>example@example.com</span>
              </div>

            </div>
          </div>
          <div className='mx-auto'>
            <Image src={'/Vector.png'} alt='fotter logo' width={296} height={100} />
            <div className='flex justify-center pt-8 gap-4 text-lg'>
              <a className='hover:text-green-300' href="#"><FaFacebookF /></a>
              <a className='hover:text-green-300' href="3"><FaInstagram /></a>
              <a className='hover:text-green-300' href="#"><FaLinkedinIn /></a>
              <a className='hover:text-green-300' href="#"><FaXTwitter /></a>
            </div>
          </div>
          <div className='mx-auto'>
            <ul className='flex flex-col gap-4 justify-end text-xl'>
              <li className=' hover:text-green-400'>
                <a href="#">Home</a>
              </li>
              <li className=' hover:text-green-400'>
                <a href="#">About</a>
              </li>
              <li className=' hover:text-green-400'>
                <a href="#">
                  contact
                </a>
              </li>
              <li className=' hover:text-green-400'>
                <a href="#">
                  Services
                </a>
              </li>
            </ul>
          </div>
        </div>



        <div className='text-center py-5 text-xs'>
          <p >Copyright 2024 &copy;SIM SUM SIM</p>
        </div>
      </div>
    </section>
  )
}

export default Footer
